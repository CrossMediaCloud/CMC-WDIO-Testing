...
function getScreenshotName(folder, context) {
	...
	return path.join(process.cwd(), 'screenshots/' + process.env.NODE_ENV, folder, `${testParent} ${testName} - ${browserName} - ${browserWidth}x${browserHeight}.png`);
}
...
exports.config = {
	...
	//
	specs: [
		'./tests/*.js' // Just the tests in first level
	],
	// Patterns to exclude.
	exclude: [
		'./tests/helper/**/*.js'
	...
	screenshotPath: './errorshots/' + process.env.NODE_ENV + '/',
	//
	...
	visualRegression: {
		...
		viewports: [{width: 1920, height: 1280,}, {width: 768, height: 1024,}, {width: 320, height: 1024,},],
